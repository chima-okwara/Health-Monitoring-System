...
HMS::HMS(HCSR04 *ultra, DFRobot_MAX30102 *pox, HX711_ADC *load, DallasTemperature *temp)
            :ultrasonic(ultra), max30102(pox), scale(load), tempSensor(temp)
{
    spo2 = 0;
    SPO2Valid = 0 ;
    heartRate = 0 ;
...
uint8_t HMS::init()
{
    ...
    max30102->sensorConfiguration(
      /*ledBrightness=*/65,           // 65-75 (balance between signal quality and power)
    /*sampleAverage=*/SAMPLEAVG_8,  // Reduced from 16 → faster response
    /*ledMode=*/MODE_MULTILED,      // Keep for SpO₂ + HR
    /*sampleRate=*/SAMPLERATE_400,  // Reduced from 800 → better noise immunity
    /*pulseWidth=*/PULSEWIDTH_411,  // Increased from 215 → better SpO₂ accuracy
    /*adcRange=*/ADCRANGE_16384     // Increased from 8192 → better dynamic range
  );
...
//        lcd.print('-');
    ...
    lcd.clear();
    lcd.home();
    lcd.print("Measuring height and");
    lcd.setCursor(0, 1);
    lcd.print("weight...");
    hms.measureHeight();
    hms.measureWeight();
